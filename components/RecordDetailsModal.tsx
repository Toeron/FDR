...
const RecordDetailsModal: React.FC<RecordDetailsModalProps> = ({ record, onClose, onAddToCart }) => {
  ...
  const nextImg = () => setImgIndex((prev) => (prev + 1) % record.imageUrls.length);
  const prevImg = () => setImgIndex((prev) => (prev - 1 + record.imageUrls.length) % record.imageUrls.length);

  const isUponRequest = record.price === 'Upon Request';
  
  // Safety: Fallback to first image if index is invalid
  const currentImageUrl = record.imageUrls[imgIndex] || record.imageUrls[0];

  return (
    <div className="fixed inset-0 z-[100] flex items-center justify-center p-4">
      <div className="absolute inset-0 bg-black/95 backdrop-blur-xl" onClick={onClose} />
      
      <div className="relative bg-[#050505] border border-zinc-800 w-full max-w-5xl max-h-[90vh] md:max-h-[95vh] overflow-hidden flex flex-col md:flex-row md:items-center shadow-[0_0_100px_rgba(0,0,0,1)] animate-in zoom-in duration-300 rounded-sm">
        ...
        <div className="w-full md:w-1/2 aspect-square md:aspect-auto md:h-full relative bg-black flex items-center justify-center group/img border-b md:border-b-0 md:border-r border-zinc-900 overflow-hidden flex-shrink-0">
          <img 
            src={currentImageUrl} 
          ...
          {hasMultipleImages && (
            <>
              <button 
                ...
                className="absolute left-4 top-1/2 -translate-y-1/2 bg-black/60 hover:bg-[#d4af37] text-white hover:text-black p-3 rounded-full transition-all backdrop-blur-md opacity-0 group-hover/img:opacity-100 border border-white/10"
              >
                <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="3" d="M15 19l-7-7 7-7"/></svg>
              </button>
              <button 
                onClick={(e) => { e.stopPropagation(); nextImg(); }} 
                className="absolute right-4 top-1/2 -translate-y-1/2 bg-black/60 hover:bg-[#d4af37] text-white hover:text-black p-3 rounded-full transition-all backdrop-blur-md opacity-0 group-hover/img:opacity-100 border border-white/10"
              >
              ...
              <div className="absolute bottom-6 left-1/2 -translate-x-1/2 flex gap-2">
                {record.imageUrls.map((_, i) => (
                  <button 
                    ...
                    className={`h-1 rounded-full transition-all ${i === imgIndex ? 'bg-[#d4af37] w-6 shadow-[0_0_8px_rgba(212,175,55,0.6)]' : 'bg-white/30 w-1'}`} 
                  />
                ))}
              </div>
            </>
        ...
        <div className="w-full md:w-1/2 p-6 md:p-12 flex flex-col overflow-y-auto custom-scrollbar relative bg-[#0a0a0a] self-stretch">
